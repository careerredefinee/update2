...
const DashboardPage: React.FC = () => {
  ...
  const isMasterRoute = location.pathname.startsWith('/master-admin');
  ...
  const renderSectionContent = () => {
    ...
    const createBar = (
      <div className="bg-white rounded-lg shadow p-4 mb-4">
        ...
        {user?.role === 'masteradmin' && section === 'awards' && (
          <form
            className="grid grid-cols-1 md:grid-cols-6 gap-3"
            onSubmit={async (e) => {
              e.preventDefault();
        ...
        {user?.role === 'masteradmin' && section === 'brands' && (
          <form
            className="grid grid-cols-1 md:grid-cols-6 gap-3"
            onSubmit={async (e) => {
              e.preventDefault();
        ...
        {user?.role === 'masteradmin' && section === 'champions' && (
          <form
            className="grid grid-cols-1 md:grid-cols-6 gap-3"
            onSubmit={async (e) => {
              e.preventDefault();
        ...
        {user?.role === 'masteradmin' && section === 'mentors' && (
          <form
            className="grid grid-cols-1 md:grid-cols-6 gap-3"
            onSubmit={async (e) => {
              e.preventDefault();
        ...
        {user?.role === 'masteradmin' && section === 'jobs' && (
          <form
            className="grid grid-cols-1 md:grid-cols-6 gap-3"
            onSubmit={async (e) => {
              e.preventDefault();
    ...
    return <>
      {createBar}
      <ManagementTable title={section} items={data} columns={(columns[section]?.includes('Actions') ? columns[section] : [...columns[section], 'Actions'])} renderRow={(item) => (
      <tr key={item._id}>
